/*************************************************************************
 > Filename   : Application_LED.c
 > Author     : oneface - one_face@sina.com
 > Company    : 一尊还酹江月
 > Time       : 2018-03-22 10:44:57
 ************************************************************************/
...
BOOL led_check_chip_id(BYTE chip_index)
{
	...
	if (NULL != p_hw_info
	    && NULL != p_hw_info->p_led_funcs && NULL != p_hw_info->p_led_funcs->p_check_chip_id && APPLI_CHIPS_NONE != p_hw_info->awchip_id) {
...
void led_chip_read_reg_cfg(BYTE chip_index)
{
	...
	p_hw_info = led_get_hw_info_by_chip_index(chip_index);
	if (NULL != p_hw_info
	    && NULL != p_hw_info->p_led_funcs && NULL != p_hw_info->p_led_funcs->p_get_regs_cfg && APPLI_CHIPS_NONE != p_hw_info->awchip_id) {
...
void led_regs_init(BYTE chip_index)
{
	...
	p_hw_info = led_get_hw_info_by_chip_index(chip_index);
	if (NULL != p_hw_info
	    && NULL != p_hw_info->p_led_funcs && NULL != p_hw_info->p_led_funcs->p_regs_init && APPLI_CHIPS_NONE != p_hw_info->awchip_id) {
...
void led_chip_sleep(BYTE chip_index)
{
	...
	p_hw_info = led_get_hw_info_by_chip_index(chip_index);
	if (NULL != p_hw_info
	    && NULL != p_hw_info->p_led_funcs && NULL != p_hw_info->p_led_funcs->p_chip_sleep && APPLI_CHIPS_NONE != p_hw_info->awchip_id) {
...
void led_chip_softreset(BYTE chip_index)
{
	...
	p_hw_info = led_get_hw_info_by_chip_index(chip_index);
	if (NULL != p_hw_info
	    && NULL != p_hw_info->p_led_funcs && NULL != p_hw_info->p_led_funcs->p_chip_softreset && APPLI_CHIPS_NONE != p_hw_info->awchip_id) {
...
void led_chip_change_imax(BYTE chip_index, BYTE iMax_level)
{
	...
	p_hw_info = led_get_hw_info_by_chip_index(chip_index);
	if (NULL != p_hw_info
	    && NULL != p_hw_info->p_led_funcs && NULL != p_hw_info->p_led_funcs->p_chip_change_imax && APPLI_CHIPS_NONE != p_hw_info->awchip_id) {
...
void led_chip_update_data(BYTE chip_index, BYTE reg_addr, BYTE reg_num, BYTE * p_value)
{
	...
	if (NULL != p_hw_info
	    ...
	    && NULL != p_hw_info->p_led_funcs->p_chip_update_data && APPLI_CHIPS_NONE != p_hw_info->awchip_id && NULL != p_value) {
...
static BYTE *leds_regs_buf_by_index(BYTE chip_index)
{
	...
	if (NULL != p_leds_hw && APPLI_CHIPS_NONE != p_leds_hw->awchip_id) {
	...
	if (led_chips > 0) {
		...
		if (NULL != pp_regs_buf) {
			...
			for (i = 0; i < led_chips; i++) {
				...
				if (NULL != p_led_hw_info && APPLI_CHIPS_NONE != p_led_hw_info->awchip_id) {
...
static BYTE *led_data_convert_to_regs_data(BYTE chip_index, const AW_LED_STRUCT * p_led_data, const AWCHIP_LEDS_HW_STRUCT * p_led_hw_info)
{
	BYTE i = 0;
	BYTE base_reg = 0;
	...
	if (chip_index < led_get_chips_number()
	    && NULL != p_led_data && NULL != p_led_hw_info) {
...
void led_effect_update_data(BYTE chip_index, AW_LED_STRUCT * p_led_data, const AWCHIP_LEDS_HW_STRUCT * p_led_hw_info)
{
	BYTE *p_regs_buf = NULL;
	...
	if (NULL != p_led_data && NULL != p_led_hw_info) {
		...
		led_chip_update_data(chip_index, p_led_hw_info->led_regs_info.reg_data_base, p_led_hw_info->led_regs_info.reg_data_nums, p_regs_buf);
	}
}
