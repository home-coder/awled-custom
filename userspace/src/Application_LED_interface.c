...
static APP_LEDS_INFO *application_get_leds_info_by_index(BYTE chip_index)
{
	...
	if (NULL != p_hw_info && APPLI_CHIPS_NONE != p_hw_info->awchip_id && NULL != p_tmp) {
...
static void application_set_led_data(AW_LED_STRUCT * p_led_data, APP_COLOR_STRUCT * p_led_color, BYTE brightness)
{
...
void application_set_led_color(BYTE chip_index, BYTE led_idx, APP_COLOR_STRUCT * p_led_color, BOOL update_led)
{
	...
	if (NULL != p_app_leds_info
	    && NULL != p_led_color
	    && led_idx < p_app_leds_info->led_nums && NULL != p_app_leds_info->p_led_data && NULL != p_app_leds_info->p_led_brightness) {
		// set color data
		memcpy((void *)(p_app_leds_info->p_led_color + led_idx), (const void *)p_led_color, sizeof(APP_COLOR_STRUCT));

		if (update_led) {
			// combine brightness and color to real brightness
			application_set_led_data(p_app_leds_info->p_led_data + led_idx, p_led_color, *(p_app_leds_info->p_led_brightness + led_idx));
		}
	}
}
...
void application_set_led_same_color(BYTE chip_index, const APP_COLOR_STRUCT * p_led_color, BOOL update_led)
{
	...
	if (NULL != p_app_leds_info
	    ...
	    && NULL != p_app_leds_info->p_led_data && NULL != p_app_leds_info->p_led_brightness && NULL != p_led_color) {
		led_nums = p_app_leds_info->led_nums;
		...
		if (start < led_nums && end < led_nums) {
			if (start <= end) {
				for (i = start; i <= end; i++) {
					// set color data
					memcpy((void *)(p_app_leds_info->p_led_color + i), (const void *)p_led_color, sizeof(APP_COLOR_STRUCT));
...
void application_set_all_same_color(BYTE chip_index, APP_COLOR_STRUCT * p_led_color, BOOL update_led)
{
	...
	if (NULL != p_app_leds_info
		...
		for (i = 0; i <= led_nums - 1; i++) {
			...
			if (update_led) {
				// combine brightness and color to real brightness
				application_set_led_data(p_app_leds_info->p_led_data + i, p_led_color, *(p_app_leds_info->p_led_brightness + i));
			}
		}
	}
}
...
void application_set_led_bright_level(BYTE chip_index, BYTE led_idx, BYTE bright_level, BOOL update_led)
{
	...
	if (NULL != p_app_leds_info
	    ...
	    && NULL != p_app_leds_info->p_led_color
	    && NULL != p_app_leds_info->p_led_data && led_idx < p_app_leds_info->led_nums && bright_level < application_get_max_bright_level()) {
...
void application_set_led_same_bright_level(BYTE chip_index, BYTE bright_level, BOOL update_led)
{
	...
	if (NULL != p_app_leds_info
	    && NULL != p_app_leds_info->p_led_bright_level
	    && NULL != p_app_leds_info->p_led_brightness
	    && NULL != p_app_leds_info->p_led_color && NULL != p_app_leds_info->p_led_data && bright_level < application_get_max_bright_level()) {
		led_nums = p_app_leds_info->led_nums;
		start = p_app_leds_info->led_range.led_effect_start;
		end = p_app_leds_info->led_range.led_effect_end;
...
void application_set_all_same_bright_level(BYTE chip_index, BYTE bright_level, BOOL update_led)
{
	...
	if (NULL != p_app_leds_info
		...
		for (i = 0; i <= led_nums - 1; i++) {
			...
			if (update_led) {
				// combine brightness and color to real brightness
				application_set_led_data(p_app_leds_info->p_led_data + i, p_app_leds_info->p_led_color + i, brightness);
			}
		}
	}
}
...
void application_set_led_brightness(BYTE chip_index, BYTE led_idx, BYTE brightness, BOOL update_led)
{
	...
	p_app_leds_info = application_get_leds_info_by_index(chip_index);
	if (NULL != p_app_leds_info
	    && NULL != p_app_leds_info->p_led_brightness && NULL != p_app_leds_info->p_led_color && NULL != p_app_leds_info->p_led_data) {
		p_app_leds_info->p_led_brightness[led_idx] = brightness;

		if (update_led) {
			// combine brightness and color to real brightness
			application_set_led_data(p_app_leds_info->p_led_data + led_idx, p_app_leds_info->p_led_color + led_idx, brightness);
		}
	}
}
