...
static void aw981x_read_register(BYTE aw981x_id, BYTE reg_addr, BYTE reg_num, BYTE *p_value)
{
	...
	if ( aw981x_id >= AWCHIP_LED_START
		&& aw981x_id < AWCHIP_LED_END )
	{
		...
		APPLICATION_ReadRegisters( EVENT_ID_AWCHIP_REG_READ_8BIT, &reg_access_data, p_regs_buf );
...
static void aw981x_write_register_ex(BYTE aw981x_id, BYTE reg_addr, BYTE reg_num, BYTE *p_value)
{
		...
		&& NULL != p_value )
	{
		...
		APPLICATION_WriteRegisters( EVENT_ID_AWCHIP_REG_WRITE_8BIT, &reg_access_data, p_value );
	}
}
