...
static void application_led_effect_increase(BYTE chip_index)
{
	...
	if ( NULL != p_led_effect && led_nums > 0 )
	{
			...
			|| (p_led_effect->last_state != p_led_effect->cur_state) )
		{
			...
			application_set_led_same_color( chip_index, 
					application_get_led_color_by_index(p_led_effect->color_level), TRUE );	
		}
	}
}
...
static void application_led_effect_change_bright(BYTE chip_index)
{
	...
	if ( NULL != p_led_effect )
	{
		...
		application_set_led_same_color( chip_index, 
			application_get_led_color_by_index(p_led_effect->color_level), FALSE );
	...
	if ( NULL != p_led_effect )
	{
		...
		if ( max_leds > 0 )
		{
			...
			for ( i = 0; i < max_color_nums; i++ )
			{
				...
				if ( j >= i )
				{
					application_set_led_bright_level( chip_index, (start+i+max_leds)%max_leds, 
								bright_level, TRUE );
				}
				// close all other leds
				else
				{
					application_set_led_brightness( chip_index, (start+i+max_leds)%max_leds, 
								0, TRUE );
				}
			}
	...
	switch ( p_led_effect->cur_state )
	{
		...
		case LEDS_EFFECT_CIRCLE:
			if ( p_led_effect->last_state != p_led_effect->cur_state )
			{
				...
				application_start_timer( application_get_effect_TIMx(), 15 );
			}
			else
			{
				// just restart TIMx for circle
				application_restart_timer( application_get_effect_TIMx() );
			}
